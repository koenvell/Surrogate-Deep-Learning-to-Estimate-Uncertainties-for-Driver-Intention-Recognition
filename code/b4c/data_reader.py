...
def b4c_parse_train_data(file_path):
    """ Requires python 2.7!!! 
    Download train data from: 
    https://github.com/asheshjain399/RNNexp/tree/7fc5a53292dc0f232867beb66c3a9ef845d705cb/anticipatory-rnn/maneuver-anticipation/checkpoints/all_new_features/
...
def b4c_parse_test_data(file_path):
    """ Requires python 2.7!!! 
    ...
    # open pickle file
    test_data = cPickle.load(open(file_path))
    ...
    for i in range(len(X_te)):
        ...
        for j, k in enumerate(test_samples):
            label = Y_te[i][j]  # identifier - timestep
            id_t = np.array([int(i), int(j), int(label)])  # id, t, label
            row = np.concatenate((id_t, k[0]))
            # add as row to df
            df = df.append(pd.DataFrame(row.reshape(1, -1), columns=col_names), ignore_index=True)
